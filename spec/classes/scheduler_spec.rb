...
describe 'autofs::scheduler' do
  ...
  describe ' by default' do
    ...
    it { should require(Package['aurora-scheduler']) }
    ...
    it  do
       ...
       should_not compile
        ...
        it { should contain_exec('init-mesos-log').with_params(:require =>  Package['aurora-scheduler'], :notify => Service['aurora-scheduler'], command => '/usr/bin/mesos-log initialize --path=/var/lib/aurora/scheduler/db && /bin/chown aurora:aurora /var/lib/aurora/scheduler/db/*', unless => '/usr/bin/test -f /var/lib/aurora/scheduler/db/CURRENT' ) }

        context 'with $aurora::master => false'
        let (:facts) { 'aurora::master' => false}
        it { should_not contain_exec('init-mesos-log').with_params(:require =>  Package['aurora-scheduler'], :notify => Service['aurora-scheduler'], command => '/usr/bin/mesos-log initialize --path=/var/lib/aurora/scheduler/db && /bin/chown aurora:aurora /var/lib/aurora/scheduler/db/*', unless => '/usr/bin/test -f /var/lib/aurora/scheduler/db/CURRENT' ) }
